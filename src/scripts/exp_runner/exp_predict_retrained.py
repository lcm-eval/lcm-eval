    ...
    for seed in seeds:
        for folder, workload_paths in workloads_by_folder.items():
            ...
            command = (f"python3 main.py "
                       ...
                       f"--model_dir {model.get_model_dir(ClusterPaths(), database, retrain=True)} "
                       f"--target_dir {model.get_eval_dir(ClusterPaths(), database, retrain=True) / folder} "
                       f"--seed {seed} ")
...
if __name__ == '__main__':
    ...
    pickup_steps = [Rsync(src=[ClusterPaths().retraining_evaluation], dest=[LocalPaths().data], put=False, update=True)]
    ...
    commands = []
    wls_to_dbs = [(Database("imdb"), EvalWorkloads.ScanCostsPercentile.imdb_idx + EvalWorkloads.ScanCostsPercentile.imdb_seq),]
