...
class PoolingConv(nn.Module):
    """
    A pooling convolution used to combine multiple messages during message passing.
    """
    ...
    def __init__(self, pooling_type=None):
        ...
        # the different pooling operations which are supported are mapped to DGL functions
        dgl_mapping = {
    ...
    def forward(self, graph=None, etypes=None, in_node_types=None, out_node_types=None, feat_dict=None):
        with graph.local_scope():
            ...
            # message passing
            graph.multi_update_all({etype: (fn.copy_u('h', 'm'), self.pooling_fn('m', 'ft')) for etype in etypes},
