...
def parse_raw_plan(analyze_plan_tuples: list, analyze: bool = True, parse: bool = True):
    ...
    for q in tqdm(run_stats.query_list):
        ...
        if not curr_explain_only:
            ...
            for analyze_plan in q.analyze_plans:
                _, ex_time, planning_time = parse_raw_plan(analyze_plan, analyze=True, parse=False)
                ex_times.append(ex_time)
                planning_times.append(planning_time)
            avg_runtime = sum(ex_times) / len(ex_times)

            # parse the plan as a tree
            analyze_plan, _, _ = parse_raw_plan(q.analyze_plans[0], analyze=True, parse=True)
        ...
        verbose_plan, _, _ = parse_raw_plan(q.verbose_plan, analyze=False, parse=True)
        ...
        assert len(tables) >  0, f"No tables found in plan {q}"
