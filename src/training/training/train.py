...
def train_model(workload_runs,
    ...
    if test_workload_runs is not None:
        for p in test_workload_runs:
            ...
            target_test_csv_paths.append(os.path.join(target_path, f'test_{filename_model}_{test_workload}.csv'))
    ...
    label_norm, feature_statistics, train_loader, val_loader, test_loaders = \
        create_zeroshot_dataloader(workload_runs, test_workload_runs, statistics_file, plan_featurization_name,
                                   database,
    ...
    csv_stats, epochs_wo_improvement, epoch, model, optimizer, metrics, finished = \
        load_checkpoint(model, target_path, filename_model, optimizer=optimizer, metrics=metrics, filetype='.pt')
    ...
    while epoch < epochs and not finished and not skip_train:
        ...
        save_checkpoint(epochs_wo_improvement, epoch, model, optimizer, target_path,
    ...
    if trial is None and test_loaders is not None:
        if not (target_path is None or filename_model is None):
            ...
            for test_path, test_loader in zip(target_test_csv_paths, test_loaders):
                ...
                validate_model(test_loader,
                               ...
                               config=config,
                               custom_batch_to=model.batch_to_func)

                save_csv([test_stats], target_path + f"_{seed}_stats.csv")
